...
import { generateAllPdfs } from '../../pdf/generatePdf'; // 👈 замінили імпорт
import { LanguageSwitcher } from '../LanguageSwitcher/LanguageSwitcher';
import { PersonalDetails } from './PersonalDetails/PersonalDetails';
import { InfoBlock } from './InfoBlock/InfoBlock';
import { GeneralQuestions } from './GeneralQuestions/GeneralQuestions';
...
export const SimpleForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      signaturePadRef.current._drawStroke = SignaturePad.prototype._drawStroke;
      signaturePadRef.current.fromData(signaturePadRef.current.toData());

      const signatureDataUrl = canvasRef.current.toDataURL('image/png');

      // 🔥 Генеруємо одразу три PDF
      const { enBlob, thBlob, cnBlob } = await generateAllPdfs(formData, signatureDataUrl);

      const formDataToSend = new FormData();
      formDataToSend.append('pdfEn', new File([enBlob], 'form_en.pdf', { type: 'application/pdf' }));
      formDataToSend.append('pdfTh', new File([thBlob], 'form_th.pdf', { type: 'application/pdf' }));
      formDataToSend.append('pdfCn', new File([cnBlob], 'form_cn.pdf', { type: 'application/pdf' })); // 👈 CN
  ...
  return (
    <form className="simpleForm" onSubmit={handleSubmit}>
      ...
      <div className="simpleForm__signatureBlock">
        <div className="simpleForm__inputs">
          <label className="simpleForm__label">{t('simpleForm.signature')}</label>
      ...
      {isSending ? (
        ...
        <button type="submit" className="simpleForm__submit">
          {t('simpleForm.submit')}
        </button>
      )}
    </form>
