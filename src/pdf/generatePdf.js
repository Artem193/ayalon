...
//   const templatePath =
//     language === 'en' ? '/pdf-templates/ayalon_en.pdf' : '/pdf-templates/ayalon_th.pdf';
...
//     const lines = wrapText(formData.answerDescription, customFont, 9, width);
...
// функція для переносу слів (англ/тай)
function wrapText(text, font, size, maxWidth) {
...
// словники для вибору шаблонів і шрифтів
const TEMPLATES = {
...
// спільна функція генерації одного PDF
async function generateSinglePdf(language, formData, signatureDataUrl) {
  const templatePath = TEMPLATES[language];
  const templateBytes = await fetch(templatePath).then((res) => res.arrayBuffer());
  const pdfDoc = await PDFDocument.load(templateBytes);

  pdfDoc.registerFontkit(fontkit);
  const fontUrl = FONTS[language];
  const fontBytes = await fetch(fontUrl).then((res) => res.arrayBuffer());
  const customFont = await pdfDoc.embedFont(fontBytes);
  ...
  Object.entries(formData).forEach(([field, value]) => {
    if (!value || !styles[field]) return;
    if (
      (field.startsWith('question') && String(value).startsWith('question')) ||
      field === 'answerDescription' ||
      field === 'nameProposer' ||
      field === 'date'
    ) return;
  ...
  if (formData.nameProposer) {
    ['nameProposer1', 'nameProposer2'].forEach((key) => {
      ...
      if (!style) return;
      const page = pages[style.page ?? 0];
      page.drawText(String(formData.nameProposer), {
        x: style.x,
        y: style.y,
        size: style.size || 9,
        font: customFont,
  ...
  if (formData.date) {
    ['date1', 'date2'].forEach((key) => {
      ...
      if (!style) return;
      const page = pages[style.page ?? 0];
      page.drawText(String(formData.date), {
        x: style.x,
        y: style.y,
        size: style.size || 9,
        font: customFont,
...
// нова функція — згенерує всі три PDF
export async function generateAllPdfs(formData, signatureDataUrl) {
  const enBlob = await generateSinglePdf('en', formData, signatureDataUrl);
  const thBlob = await generateSinglePdf('th', formData, signatureDataUrl);
  const cnBlob = await generateSinglePdf('cn', formData, signatureDataUrl);
  return { enBlob, thBlob, cnBlob };
}
