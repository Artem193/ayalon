...
export async function generatePdf(language, formData, signatureDataUrl) {
  ...
  Object.entries(formData).forEach(([field, value]) => {
    ...
    const page = pages[st.page ?? 0];
    safeDrawMixedText(page, sanitizeInput(value), st.x, st.y, st.size || 9, fonts);
  });
  ...
  if (formData.nameProposer) {
    for (const key of ['nameProposer1', 'nameProposer2']) {
      ...
      if (!st) continue;
      const page = pages[st.page ?? 0];
      safeDrawMixedText(page, sanitizeInput(formData.nameProposer), st.x, st.y, st.size || 9, fonts);
    }
  }
  ...
  if (formData.date) {
    for (const key of ['date1', 'date2']) {
      ...
      if (!st) continue;
      const page = pages[st.page ?? 0];
      safeDrawMixedText(page, sanitizeInput(formData.date), st.x, st.y, st.size || 9, fonts);
    }
  }
