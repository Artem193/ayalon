...
const FONTS = {
  en: '/fonts/NotoSans-Regular.ttf',       // латиниця + кирилиця (англійська, російська)
  he: '/fonts/NotoSansHebrew-Regular.ttf', // іврит
  th: '/fonts/NotoSansThai-Regular.ttf',   // тайська
  cn: '/fonts/NotoSerifCJKsc-Regular.otf', // китайська
};

// Визначаємо шрифт для тексту по символах
function detectFontForText(text, fonts) {
  for (const ch of text) {
    const code = ch.charCodeAt(0);

    if (code >= 0x0590 && code <= 0x05FF) return fonts.he; // іврит
    if (code >= 0x0400 && code <= 0x04FF) return fonts.en; // кирилиця (російська)
    if (code >= 0x0E00 && code <= 0x0E7F) return fonts.th; // тайська
    if (code >= 0x4E00 && code <= 0x9FFF) return fonts.cn; // китайська
  }
  return fonts.en; // дефолт (англійська/латиниця)
}
...
export async function generatePdf(language, formData, signatureDataUrl) {
  ...
  const [fontEnBytes, fontHeBytes, fontThBytes, fontCnBytes] = await Promise.all([
  ...
  const fonts = {
    en: await pdfDoc.embedFont(fontEnBytes, { subset: true }),
    he: await pdfDoc.embedFont(fontHeBytes, { subset: true }),
    th: await pdfDoc.embedFont(fontThBytes, { subset: true }),
    cn: await pdfDoc.embedFont(fontCnBytes, { subset: true }),
  };

  const pages = pdfDoc.getPages();
  const styles = pdfStyles[language] || pdfStyles.en;

  // 1. Вставка текстовых полей
  Object.entries(formData).forEach(([field, value]) => {
    if (!value || !styles[field]) return;
    ...
    const fontToUse = detectFontForText(value, fonts);
  ...
  // 1.0 nameProposer — отрисовка в двух местах
  if (formData.nameProposer) {
    const namePositions = ['nameProposer1', 'nameProposer2'];
    ...
    namePositions.forEach((key) => {
      ...
      const page = pages[style.page ?? 0];
      const fontToUse = detectFontForText(formData.nameProposer, fonts);
  ...
  if (formData.date) {
    const namePositions = ['date1', 'date2'];
    ...
    namePositions.forEach((key) => {
      ...
      const page = pages[style.page ?? 0];
      const fontToUse = detectFontForText(formData.date, fonts);
  ...
  if (formData.answerDescription && styles.answerDescription) {
    ...
    const fontToUse = detectFontForText(formData.answerDescription, fonts);
    const lines = (language === 'cn')
      ? wrapTextCJK(formData.answerDescription, fontToUse, 9, width)
      : wrapText(formData.answerDescription, fontToUse, 9, width);
    const page = pages[pageIndex];
